...
case $desktop in
    ...
    if type "xrandr" > /dev/null; then
      for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
        [[ $m = 'eDP1' ]] && pos="right" || pos="none"
        echo $pos
        MONITOR=$m TRAY_POS=$pos polybar --reload mainbar-i3 -c ~/.config/polybar/config &
      done
    else
    ...
    bspwm)
    if type "xrandr" > /dev/null; then
      for m in $(xrandr --countquery | grep " connected" | cut -d" " -f1); do
